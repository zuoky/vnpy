...
class BacktestingEngine(object):
    ...
    def loadHistoryData(self):
        ...
        # 载入初始化需要用的数据
        flt = {'TradingDay': {'$gte': int(self.dataStartDate.strftime("%Y%m%d")),
                              '$lt': int(self.strategyStartDate.strftime("%Y%m%d"))}}
        ...
        if not self.dataEndDate:
            flt = {'TradingDay': {'$gte': int(self.strategyStartDate.strftime("%Y%m%d"))}}  # 数据过滤条件
        else:
            flt = {'TradingDay': {'$gte': int(self.strategyStartDate.strftime("%Y%m%d")),
                                  '$lte': int(self.dataEndDate.strftime("%Y%m%d"))}}
    ...
    def runBacktesting(self):
        ...
        if self.mode == self.BAR_MODE:
            ...
            for d in self.dbCursor:
                data = CtaTickData()
                data.vtSymbol = d["InstrumentID"]
                data.symbol = d["InstrumentID"]
                data.exchange = d["ExchangeID"]

                data.lastPrice = d["LastPrice"]
                data.volume = d["Volume"]
                data.openInterest = d["OpenInterest"]
                data.upperLimit = d["UpperLimitPrice"]
                data.lowerLimit = d["LowerLimitPrice"]

                data.date = str(d["TradingDay"])
                data.time = "{}.{}000".format(d["UpdateTime"], str(d["UpdateMillisec"]))
                data.datetime = datetime.strptime("{} {}".format(data.date, data.time), "%Y%m%d %H:%M:%S.%f")

                data.bidPrice1 = d["BidPrice1"]
                data.bidPrice2 = d["BidPrice2"]
                data.bidPrice3 = d["BidPrice3"]
                data.bidPrice4 = d["BidPrice4"]
                data.bidPrice5 = d["BidPrice5"]

                data.askPrice1 = d["AskPrice1"]
                data.askPrice2 = d["AskPrice2"]
                data.askPrice3 = d["AskPrice3"]
                data.askPrice4 = d["AskPrice4"]
                data.askPrice5 = d["AskPrice5"]

                data.bidVolume1 = d["BidVolume1"]
                data.bidVolume2 = d["BidVolume2"]
                data.bidVolume3 = d["BidVolume3"]
                data.bidVolume4 = d["BidVolume4"]
                data.bidVolume5 = d["BidVolume5"]

                data.askVolume1 = d["AskVolume1"]
                data.askVolume2 = d["AskVolume2"]
                data.askVolume3 = d["AskVolume3"]
                data.askVolume4 = d["AskVolume4"]
                data.askVolume5 = d["AskVolume5"]
